...
export class TaskShareUseCases {
  ...
  shareTask = async ( taskUUID, usersUUIDS ) => {
    try {
      ...
      return new Error("Debes enviar un arreglo de usuarios con al menos 1 usuario a compartir")
      await this.taskUseCases.findTask(taskUUID)
      const sharedTask = await this.shareTaskLoop(
        usersUUIDS,
        taskUUID
      )
      if (!Array.isArray(sharedTask) || sharedTask.length === 0)
      return new Error("No se logro compartir la tarea con todos los usuarios")
  ...
  toDoResponsible = async (userUUID, taskUUID) => {
    try {
      const taskSharedRelation = await this.taskshareRepository.getRelation(userUUID,taskUUID)
      if (!taskSharedRelation) {
        return new Error(`La terea no Esta Compartida con el usuario: ${userUUID}`)
      }
      const userResponsible = await this.taskshareRepository.getResponsible(taskUUID)
      if(userResponsible && userResponsible.userUUID !== userUUID)
      {
        await this.taskshareRepository.updateResponsible(userResponsible.userUUID,false)
  ...
  AllUsersByTask = async (taskUUID) => {
    try {
      ...
      return new Error(`No hay usuarios Compartidos para la Tarea :  ${taskUUID}`)
      const users =await this.resolveArrayPromises(sharedUsers.map(async (user) => {
      return {isResponsible: user.responsible, user:await this.usersUseCases.findUser(user.userUUID)}}))
      return users
    } catch (error) {
  ...
  getTaskSharedPromises = async (TasksShared) => {
    try {
      const TasksDataArray = TasksShared.map(async (Task) => {
        return{UsersShared :Task.usersShared, Task: await this.taskUseCases.findTask(Task.taskUUID)}
      })
  ...
  resolveArrayPromises = async (ArrayPromises) => {
    try {
      ...
      if (PromiseResolved instanceof Error) throw new Error(PromiseResolved.message)
